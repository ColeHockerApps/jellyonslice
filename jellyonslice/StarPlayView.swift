...
struct StarPlayView: UIViewRepresentable {

    let startPoint: URL
    let launch: StarLaunchPoints
    let orientation: StarOrientationManager
    let onReady: () -> Void
...
final class Handler: NSObject, WKNavigationDelegate, WKUIDelegate {
    ...
    private let launch: StarLaunchPoints
    private let orientation: StarOrientationManager
    ...
    init(
        startPoint: URL,
        launch: StarLaunchPoints,
        orientation: StarOrientationManager,
        ...
        self.baseHost = startPoint.host?.lowercased()
    }
